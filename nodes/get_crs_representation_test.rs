// Separate test file: nodes/get_crs_representation_test.rs. The generated service wires
// it into the crate via `#[cfg(test)] #[path="nodes/get_crs_representation_test.rs"] mod
// get_crs_representation_test;`. It reaches the node + SDK through `crate::` paths (this is
...
mod tests {
    ...
    use crate::get_crs_representation::get_crs_representation;
    ...
    // Independent oracle: EPSG:4326's PROJ4 string is a widely published,
    // stable definition ("+proj=longlat +datum=WGS84 ...").
    #[test]
    fn test_wgs84_proj4_matches_published_definition() {
        let ax = test_context();
        let out = get_crs_representation(&ax, q("EPSG:4326")).unwrap();
        assert_eq!(out.error, "");
        assert!(out.proj4.contains("+proj=longlat"), "proj4={}", out.proj4);
        assert!(out.proj4.contains("+datum=WGS84"), "proj4={}", out.proj4);
        assert!(out.wkt.starts_with("GEOGCRS[\"WGS 84\""), "wkt={}", out.wkt);
        assert!(out.wkt.contains("ID[\"EPSG\",4326]"), "wkt={}", out.wkt);
    }

    #[test]
    fn test_web_mercator_wkt_is_projcrs() {
        let ax = test_context();
        let out = get_crs_representation(&ax, q("3857")).unwrap();
        assert_eq!(out.error, "");
        assert!(out.wkt.starts_with("PROJCRS["), "wkt={}", out.wkt);
        assert!(out.proj4.contains("+proj=merc"), "proj4={}", out.proj4);
    }

    #[test]
    fn test_unknown_epsg_code_is_structured_error() {
        let ax = test_context();
        let out = get_crs_representation(&ax, q("EPSG:1")).unwrap();
