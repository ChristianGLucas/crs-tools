// Separate test file: nodes/validate_crs_test.rs. The generated service wires
// it into the crate via `#[cfg(test)] #[path="nodes/validate_crs_test.rs"] mod
// validate_crs_test;`. It reaches the node + SDK through `crate::` paths (this is
...
mod tests {
    ...
    use crate::validate_crs::validate_crs;
    ...
    #[test]
    fn test_known_epsg_code_is_valid() {
        let ax = test_context();
        let out = validate_crs(&ax, q("EPSG:4326")).unwrap();
        assert!(out.valid);
    ...
    #[test]
    fn test_bare_digits_are_valid() {
        let ax = test_context();
        let out = validate_crs(&ax, q("3857")).unwrap();
        assert!(out.valid);
        assert_eq!(out.epsg_code, "EPSG:3857");
    }

    #[test]
    fn test_raw_proj4_string_is_valid() {
        let ax = test_context();
        let out = validate_crs(&ax, q("+proj=longlat +datum=WGS84 +no_defs")).unwrap();
        assert!(out.valid);
        assert_eq!(out.reason, "");
    }

    #[test]
    fn test_unknown_epsg_code_is_invalid_not_a_crash() {
        let ax = test_context();
        let out = validate_crs(&ax, q("EPSG:1")).unwrap();
        assert!(!out.valid);
        assert_eq!(out.reason, "UNKNOWN_EPSG_CODE");
    }

    #[test]
    fn test_garbage_string_is_invalid_not_a_crash() {
        let ax = test_context();
        let out = validate_crs(&ax, q("this is not a crs at all")).unwrap();
        assert!(!out.valid);
        assert_eq!(out.reason, "UNPARSEABLE");
    }

    #[test]
    fn test_empty_string_is_invalid_not_a_crash() {
        let ax = test_context();
        let out = validate_crs(&ax, q("")).unwrap();
        assert!(!out.valid);
        assert_eq!(out.reason, "EMPTY_INPUT");
    }

    #[test]
    fn test_bad_proj4_string_is_invalid_not_a_crash() {
        let ax = test_context();
        let out = validate_crs(&ax, q("+proj=totallyNotARealProjection")).unwrap();
        assert!(!out.valid);
        assert_eq!(out.reason, "INVALID_CRS");
    }
}
